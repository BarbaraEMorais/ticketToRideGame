...
func _repor_uma_carta_na_exposta() -> void:
	...
	if is_instance_valid(nova_carta):
		...
		#if not nova_carta.carta_clicada.is_connected(callable_method):
		var err = nova_carta.carta_clicada.connect(callable_method)  
			#if err == OK:
			#	print("PilhaExposta (CONEXÃO): SINAL CONECTADO de '%s' para _on_carta_exposta_foi_clicada (usando Callable)." % nova_carta.name)
			#else:
			#	print("PilhaExposta (CONEXÃO): ERRO AO CONECTAR SINAL de '%s' com Callable. Código de erro: %s" % [nova_carta.name, err])
...
func _atualizar_posicoes_cartas() -> void:
	...
	if has_node("Area2D/CollisionShape2D"):
		var shape_node = get_node("Area2D/CollisionShape2D") as CollisionShape2D
		if shape_node and shape_node.shape is RectangleShape2D:
			...
			rect_shape.size = Vector2(total_width_area, total_height_area)
